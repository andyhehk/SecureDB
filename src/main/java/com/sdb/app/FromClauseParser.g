...
@after {gParent.msgs.pop(); }
	:
	KW_COALESCE fromSource -> ^(TOK_COALESCE fromSource)
	;
...
multipleTrack
    : multipleSource (COMMA multipleSource)+ -> ^(TOK_MULTIPLETRACK multipleSource multipleSource+)
    ;
...
exclusivenessJoinSource
	: KW_EXCLUSIVENESSJOIN LPAREN fromSource COMMA fromSource RPAREN (KW_BY aggregation)? -> ^(TOK_EXCLUSIVENESSJOIN fromSource fromSource aggregation?)
	;
	
intersectJoinSource
	: KW_INTERSECTJOIN LPAREN fromSource (COMMA fromSource)+ RPAREN (KW_BY aggregation)? -> ^(TOK_INTERSECTJOIN fromSource fromSource+ aggregation?)
	;
...
@after {gParent.msgs.pop(); }
    :
    KW_PROJECT KW_VIRTUALTRACK intervalLength KW_ON fromSource -> ^(TOK_PROJECT intervalLength fromSource)
    |
    KW_PROJECT fromSource KW_ON fromSource -> ^(TOK_PROJECT fromSource fromSource)
    ;
